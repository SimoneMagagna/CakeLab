// Modifica i dati di un amministratore già inserito.
<?php
...
if(isset($_POST['modC'])){
	...
	if(!$isErrore){
		try{
			//Modifico i dati
			if (!($stmt = $mysqli->prepare("CALL updAdmin( ?,?,?,?,?,@ris,@tipoErrore);")))
				throw new Exception ('CALL fallita: (' . $mysqli->errno . ') ' . $mysqli->error);   
			   
			if (!($stmt->bind_param("issss", $_POST['idC'], $_POST['userC'], $_POST['nomeC'], $_POST['cognomeC'],  $_POST['emailC'])))
	...
	//Modifico la password solo se il campo non è stato lasciato vuoto
				if (!($stmt = $mysqli->prepare("UPDATE ADMIN SET passwordAdmin = ? WHERE idAdmin = ?;")))
	...
	$isAvvisi = TRUE;
	if($risultato){
		$avvisi = "Modifica dell'Admin riuscita!";
	}else{
		$avvisi .= $tipoErrore;
	}
...
if(isset($_GET['mod'])){ 
	...
	try{
		...
		if (!($stmt = $mysqli->prepare("SELECT idAdmin, userAdmin, nomeAdmin, cognomeAdmin, mailAdmin FROM ADMIN WHERE idAdmin=?;")))
	...
	if (!($stmt->bind_result($_POST['idC'], $_POST['userC'], $_POST['nomeC'], $_POST['cognomeC'], $_POST['emailC'])))
		throw new Exception ('bind_result fallita: (' . $mysqli->errno . ') ' . $mysqli->error);
...
<head>
<meta charset="UTF-8">
<title>The CakeLab - Modifica Admin</title>
...
<fieldset>
  	<legend>Modifica i dati dell'admin</legend>
  ...
  <fieldset>
    ...
    <input class="mainForm" type="password" id="ripPwdC" name="ripPwdC" size="10" maxlength="255">
    <input class="mainForm" type="hidden" id="idC" name="idC" size="10" maxlength="255" value="<?php echo $_POST['idC'];?>"></div>
   </fieldset>
   
 
    <button type="submit" id="modC" name="modC" value="Modifica Admin">Modifica Admin</button>
